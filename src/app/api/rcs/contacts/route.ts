...
export async function GET(request: NextRequest) {
  ...
  if (campaignId) {
    const campaign = await RCSCampaign.findOne({ _id: campaignId, companyId: ctx.companyId }).select("audienceContactIds");
    query._id = { $in: campaign?.audienceContactIds || [] };
  }

  const { skip } = paginate(page, limit);
  const [contacts, total] = await Promise.all([
    RCSContact.find(query).skip(skip).limit(limit).sort({ createdAt: -1 }),
    RCSContact.countDocuments(query),
  ]);
...
export async function POST(request: NextRequest) {
  ...
  const { name, phone, tags, optIn } = body;
  if (!phone) return apiError("Phone is required");

  const existing = await RCSContact.findOne({ companyId: ctx.companyId, phone });
  if (existing) return apiError("A contact with this phone number already exists");

  const contact = await RCSContact.create({
