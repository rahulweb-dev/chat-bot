...
export async function POST(request: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!rawPhone) {
      results.push({ row: i + 2, phone: rawPhone, tags, status: "INVALID", reason: "Missing phone number" });
      continue;
    }
    if (!isValidPhone(phone)) {
      results.push({ row: i + 2, name, phone: rawPhone, tags, status: "INVALID", reason: "Invalid phone number format" });
      continue;
    }
    ...
    if (existing) {
      ...
      const created = await RCSContact.create({
    ...
    results.push({ row: i + 2, name, phone, tags, status: "VALID" });
  }
